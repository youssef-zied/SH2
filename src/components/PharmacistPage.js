...
export class PharmacistPage extends Component {
    ...
    selectPrescription = (e, patientId) => {
        e.preventDefault();
        this.setState({ 
            term: e.target.name,
        });
    ...
    render() {
        console.log(this.state.term)
        let patientId = this.props.prescriptions.map (x=> {return ({patient:x.patientId})})
        if(this.props.prescriptions.patientId==term) {
        var Press = this.props.prescriptions.map (x=> {
            switch (x.patientId) {
...
//        let patientId = this.state.patientId,
        ...
        if (term.length > 0) {
            patientId = patientId.filter((l) => {
                return l.patient.toLowerCase().match(term);
            });
        }
        if (term.length > 3) {
            var patientIdList = patientId.map((l) => {
                    ...
                    <form className='form-inline'>
                        ...
                        <div className="card-body">
                            <div className='card text-center' style={{ minWidth: '30%', marginRight:'5%', marginLeft:'5%' }}>
                                <h5 className="card-header">{Press[0].doctorName}</h5>
                        ...
                        <div className="card-body">
                            <div className='card text-center' style={{ minWidth: '30%', marginLeft: '5%', marginRight:'5%' }}>
                                <h5 className="card-header">{Press[0].patientName}</h5>
